...
from library.chat_support import (
    calc_percentage,
    calc_progress,
    save_target_cfg,
    get_time,
...
async def clone_medias(bot: Bot, m: Message):
    ...
    start_time = datetime.datetime.now(pytz.timezone("Asia/Kolkata")).strftime(
        "%I:%M %p"
    )
    #
    ...
    delay = (
        ...
        doc
    ) = video = audio = text = voice = photo = total_copied = matching = msg_id = int()
    #
    ...
    for offset in reversed(
        range(
            end_id + 1,
            (1 if not bool(start_id) else start_id) - 1,
            (start_id - 1) if not bool(start_id) else -1,
        )
    ):
        async for user_message in bot.USER.get_chat_history(
            chat_id=source_chat, offset_id=offset, limit=1
        ):
            if not user_message.empty:
                messages = await bot.USER.get_messages(
                    source_chat, user_message.id, replies=0
                )
                msg_id = messages.id
                #
                ...
                if id not in clone_cancel_key:
                    ...
                    await msg.edit(
                        Presets.CANCELLED_MSG, reply_markup=reply_markup_finished
                    )
                    await bot.USER.send_message(
                        "me", report, disable_web_page_preview=True
                    )
                    await reset_all(id)
                    file_types.clear()
                ...
                for file_type in file_types:
                    ...
                    if media is not None:
                        ...
                        if (uid is not None) and (uid in master_index):
                            matching += 1
                            await m.edit(
                                Presets.DUPLICATE_INDEX.format(matching, msg_id)
                            )
                        # if the duplicate file is not found while cloning
                            ...
                            if uid is not None:
                                master_index.append(
                                    uid
                                )  # The unique id of the file is added to the master index list
                            ...
                            time_taken = time.strftime(
                                "%Hh %Mm", time.gmtime(time.time() - clone_start_time)
                            )
                            ...
                            try:
                                ...
                                await msg.edit_text(
                                    Presets.COPY_ERROR,
                                    reply_markup=reply_markup_finished,
                                )
                                await reset_all(id)
                                file_types.clear()
                                file_types.extend(Presets.FILE_TYPES)
                            ...
                            try:
                                await msg.edit(
                                    "🇮🇳 | " + progress if pct <= 100 else Presets.BLOCK,
