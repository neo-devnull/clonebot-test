...
async def text_update_or_terminate(client: Bot, message: Message):
    ...
    if message.forward_from_chat:
        ...
        await message.reply_text(
            Presets.GET_CHAT_ID_MSG.format(chat_id, message_id),
        ...
        if message.from_user.id == int(session.id):
            await message.reply_text(
                Presets.SESSION_START_INFO.format(start_date, time_now),
    ...
    if message.reply_to_message_id == a:
        try:
            ...
            await bot_msg.edit_text(
                Presets.WELCOME_TEXT, reply_markup=reply_markup_home
            )
            return
